...
appEvent st ev =
    case ev of
        ...
        _ -> M.continue =<< T.handleEventLensed st (currentEditor st) ev

initialState :: St
initialState =
    St (F.focusRing [firstEditor, secondEditor])
...
theApp =
    M.App { M.appDraw = drawUI
          ...
          , M.appChooseCursor = (\st cursors -> F.focusRingCursor (\s -> s^.focusr) st cursors)
